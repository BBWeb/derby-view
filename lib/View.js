...
function View(views, path, viewName, viewFn) {
  ...
  this.queryPerLevelFromSegments = null;
...
View.prototype.queryPerLevel = function (from) {
  ...
  this.queryPerLevelFromSegments = from.split('.');
...
View.prototype.destroy = function() {
  ...
  this.model._del(this.querySegments);
  this.model._del(this.keysSegments);

  // Destroy all queries
  for(var i = this.queries.length - 1; i >= 0; i--) {
    // No need to take into consideration whether it is a queryPerLevel or not since the view is also being destroyed
    this.queries[i].destroy();
  }

  if(this.fromSegments) this.model._removeRef(this.fromSegments);
};
