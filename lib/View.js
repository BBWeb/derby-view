...
View.prototype._commitDiff = function(diff) {
  ...
  function del(key) {
    ...
    if(keySegments.length > 1) {
      ...
      while(!done && segments !== view.pathsSegments && (latestSegment = segments.pop())) {
        if(Object.keys(rootModel._get(segments) || {}).length > 1) done = true;
      }
  ...
  function delQueryPerLevel(key) {
    ...
    if(!view.model._get(segments)) {
      // TODO: Add proper deletion of queries which cleans up properly
      delete this.queriesPerLevelFromMap[category];
    }
  }
  ...
  function addQueryPerLevel(key, path) {
    ...
    if(!this.queriesPerLevelFromMap[category]) {
      var query = this.query(category, category + 'z');
      query.ref(from + '.' + category);
      this.queriesPerLevelFromMap[category] = query;
    }
  }

  if(this.queryPerLevelInitialized) {
